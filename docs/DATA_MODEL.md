...
請假與公假紀錄保存 `follow_up_due_at`。到下一次該科上課仍未交時，狀態可轉為未完成、未攜帶或遲交，並開始計數。「需補考」維持待處理提醒，但不計入未完成、未攜帶或遲交次數；教師完成處理後顯示為「已補考」，並依既有規則保留 1 天。
...
`student_learning_levels` 以學生與學習系統為唯一組合，保存目前等級及連續達標次數。自然科只以每日專注任務計算進階：入門連續 3 次、進階連續 5 次達到 80 分後升級；未達 80 分時連續次數歸零。入門任務最多安排 5 個，第 6 個新任務起強制改為進階。自由練習及已完成任務的重複作答不累加，入門自由練習也不能完成進階每日任務；管理員可手動調整等級並重設進度。
...
| `learning_systems` | 各科入口、每週任務次數、對象及啟用狀態 |
| `learning_activities` | 各學習系統的遊戲／活動、題數、目標分數及啟用狀態 |
| `student_focus_tasks` | 學生每日專注任務、題數、完成狀態與最佳成績 |
| `focus_task_attempts` | 每次專注任務作答及分數紀錄 |
| `student_learning_levels` | 各學生在支援分級的學習系統中，目前等級及連續達標次數 |
